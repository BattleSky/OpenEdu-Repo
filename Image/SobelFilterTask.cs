...
namespace Recognizer
{
    internal static class SobelFilterTask
    {
        ...
        public static double Multiplicate(double[,] g, double[,] sxy, int shift, int x, int y)
        {
            var sxWidth = sxy.GetLength(0);
            var multiplication = 0.0;
            for (var i = 0; i < sxWidth; i++)
            for (var j = 0; j < sxWidth; j++)
                multiplication += sxy[i, j] * g[x - shift + i, y - shift + j];
        ...
        public static double[,] SobelFilter(double[,] g, double[,] sx)
        {
            ...
            // минимальное смещение от границ матрицы-изображения (на половину матрицы-маски), чтобы не уползать в outofrange
            ...
            {
                var gx = Multiplicate(g, sx, shift, x, y);
                var gy = Multiplicate(g, sy, shift, x, y);
